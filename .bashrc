...
alias gitl="git log --color --graph --pretty=format:'%C(auto)%h%d %s %C(black)%C(bold)%an, %ai' --abbrev-commit --decorate"
alias gitlt="git log --all --color --graph --pretty=format:'%C(white)%ai %C(auto)%h%d %s %C(black)%C(bold)%an' --abbrev-commit --decorate"
