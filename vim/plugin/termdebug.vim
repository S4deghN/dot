...
var saved_fn_map: list<dict<any>>
...
def InitScriptVariables()
  ...
  for fn_key in ['<F1>', '<F2>', '<F3>', '<F4>', '<F5>', '<F6>', '<F7>', '<F8>', '<F9>', '<F10>', '<F11>', '<F12>']
    var nmap = maparg(fn_key, 'n', false, true)
    if len(nmap) > 0
      add(saved_fn_map, nmap)
    else
      add(saved_fn_map, {lhs: fn_key, mode: 'n'})
    endif

    var tmap = maparg(fn_key, 't', false, true)
    if len(tmap) > 0
      add(saved_fn_map, tmap)
    else
      add(saved_fn_map, {lhs: fn_key, mode: 't'})
    endif
  endfor
...
silent! prop_type_add('TermdebugHeader', {
  highlight: 'Type',
})
def HandleVariablesMsg(msg: string)
  ...
  if win_gotoid(varwin)
    ...
    setline(1, '')
    prop_add(1, 1,
      {type: 'TermdebugHeader', text: $'Name{spaces}Value{spaces[: -2]}Type'})
    var cnt = 1
    ...
    while varinfo != ''
      ...
      setline(cnt + 1, vardict['name'] ..
        repeat(' ', max([20 - len(vardict['name']), 1])) ..
        vardict['value'] ..
        repeat(' ', max([20 - len(vardict['value']), 1])) ..
        vardict['type'])
      cnt += 1
...
def InstallCommands()
  ...
  map = true
  if exists('g:termdebug_config')
    map = get(g:termdebug_config, 'map_fn', true)
  endif
  if map
      # TODO: Add keybinding for Start and Run. I need to learn how to send
      # those using GDB/MI
      nnoremap <F3> <cmd>Break<CR>
      tnoremap <F3> <cmd>Break<CR>
      nnoremap <F4> <cmd>Stop<CR>
      tnoremap <F4> <cmd>Stop<CR>
      nnoremap <F5> <cmd>Over<CR>
      tnoremap <F5> <cmd>Over<CR>
      nnoremap <F6> <cmd>Step<CR>
      tnoremap <F6> <cmd>Step<CR>
      nnoremap <F7> <cmd>Finish<CR>
      tnoremap <F7> <cmd>Finish<CR>
      nnoremap <F8> <cmd>Continue<CR>
      tnoremap <F8> <cmd>Continue<CR>
  endif
...
def InstallWinbar(force: bool)
  ...
  if has('menu') && &mouse != '' && (winbar || force)
    nnoremenu WinBar.Stop(F4)   :Stop<CR>
    nnoremenu WinBar.Next(F5)   :Over<CR>
    nnoremenu WinBar.Step(F6)   :Step<CR>
    nnoremenu WinBar.Finish(F7) :Finish<CR>
    nnoremenu WinBar.Cont(F8)   :Continue<CR>
...
def DeleteCommands()
  ...
  echom saved_fn_map
  for map in saved_fn_map
    if has_key(map, 'rhs') && !map.buffer
      mapset(map)
    elseif !has_key(map, 'buffer') # indicates that it is our own default dict
      exe 'silent!' map.mode .. 'unmap' map.lhs
    endif
  endfor

  # # TODO: save the mappings for fn keys like above.
  # silent! nunmap <F3>
  # silent! tunmap <F3>
  # silent! nunmap <F4>
  # silent! tunmap <F4>
  ...
  if has('menu')
    ...
    for winid in winbar_winids
      if win_gotoid(winid)
        aunmenu WinBar.Stop(F4)
        aunmenu WinBar.Next(F5)
        aunmenu WinBar.Step(F6)
        aunmenu WinBar.Finish(F7)
        aunmenu WinBar.Cont(F8)
        aunmenu WinBar.Eval
      endif
    endfor
    win_gotoid(curwinid)
